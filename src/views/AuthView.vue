      ...
      <div :class="s.right()">
        ...
        <div v-if="success" :class="s.successWrap()" style="animation: fadeUp 400ms ease both">
          ...
          <div :class="s.successSub()">
            {{ mode === 'forgot' ? 'Проверьте почту и перейдите по ссылке для сброса пароля' : mode === 'register' ? 'Перейдите на почту и активируйте свой личный кабинет' : 'Добро пожаловать в панель управления' }}
          </div>
        </div>
